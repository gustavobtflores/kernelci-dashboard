...
from kernelCI_app.helpers.treeDetailsRollup import (
    decide_if_build_dimension_filtered_out,
    decide_if_rollup_row_filtered_out,
...
class BaseTreeDetailsSummary(APIView):
    ...
    def _sanitize_builds_rows(self, builds_rows: list[dict]) -> None:
        for row_dict in builds_rows:
            ...
            if decide_if_build_dimension_filtered_out(self, row_data):
                continue
    ...
    def _sanitize_rollup_rows(self, rollup_rows: list[dict]) -> None:
        for row_dict in rollup_rows:
            ...
            if decide_if_rollup_row_filtered_out(self, row_dict):
                continue

            is_boot_row = row_dict["is_boot"]

            if rollup_test_or_boot_filtered_out(
                self, row_dict, is_boot_row=is_boot_row
            ):
                continue

            process_rollup_issues(self, row_dict, is_boot_row=is_boot_row)
            process_rollup_summary(self, row_dict, is_boot_row=is_boot_row)
