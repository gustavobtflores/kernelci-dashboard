...
def decide_if_build_dimension_filtered_out(instance, row_data: dict) -> bool:
...
def process_rollup_filters(instance, row_dict: dict) -> None:
    ...
    incident_test_id = "rollup_test" if issue_id is not None else None
    ...
    test_lab = row_dict.get("test_lab", UNKNOWN_STRING)
    origin_set.add(test_origin)
    lab_set.add(test_lab or UNKNOWN_STRING)


def decide_if_rollup_row_filtered_out(instance, row_dict: dict) -> bool:
    """Check if a rollup row should be filtered out by dimension-level filters."""
    return instance.filters.is_record_filtered_out(
