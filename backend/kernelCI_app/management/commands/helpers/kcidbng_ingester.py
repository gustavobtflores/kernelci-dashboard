                ...
                with connection.cursor() as cursor:
                    ...
                    if builds_buf:
                        ...
                        for build in builds_buf:
                            cursor.execute(
                                "INSERT INTO new_build (build_id, checkout_id, build_origin, status) VALUES (%s, %s, %s, %s)",
                                (
                                    build.id,
                                    build.checkout_id,
                                    build.origin,
                            ...
                            cursor.execute(
                                "INSERT INTO hardware_status (hardware_origin, hardware_platform, hardware_model, compatibles, checkout_id, date) VALUES (%s, %s, %s, %s, %s, %s)",
                                (
                                    build.origin,
                    ...
                    if tests_buf:
                        ...
                        for test in tests_buf:
                            pass_count = 0 if test.status == "PASS" else 1
                            failed_count = 0 if test.status == "FAIL" else 1
                            inc_count = (
                                0
                                if test.status != "PASS" and test.status != "FAIL"
                                else 1
                            )

                            if test.path == "boot" or test.path.startswith("boot."):
                                cursor.execute(
                                    "INSERT INTO new_boot (boot_id, build_id, boot_origin, status) VALUES (%s, %s, %s, %s)",
                                    (test.id, test.build_id, test.origin, test.status),
                                )

                                cursor.execute(
                                    "INSERT INTO boots_status (build_id, pass, failed, inc) VALUES (%s, %s, %s, %s) ON CONFLICT (build_id) DO UPDATE SET pass = boots_status.pass + %s, failed = boots_status.failed + %s, inc = boots_status.inc + %s",
                                    (
                                        test.build_id,
                                        pass_count,
                                        failed_count,
                                ...
                                cursor.execute(
                                    "INSERT INTO new_test (test_id, build_id, test_origin, status) VALUES (%s, %s, %s, %s)",
                                    (test.id, test.build_id, test.origin, test.status),
                                )

                                cursor.execute(
                                    "INSERT INTO tests_status (build_id, pass, failed, inc) VALUES (%s, %s, %s, %s) ON CONFLICT (build_id) DO UPDATE SET pass = tests_status.pass + %s, failed = tests_status.failed + %s, inc = tests_status.inc + %s",
                                    (
                                        test.build_id,
                                        pass_count,
                                        failed_count,
